...
def build_report(rule_sets: dict[str, dict], llm_sets: dict[str, dict] | None, datasets: dict) -> str:
    ...
    lines.append(
        f"| robustness | {datasets['robustness']} | 口语化改写 / 同义替换 / 隐私与注入的变体表述，用于检验规则泛化性 |"
    )
    lines.append("")
    ...
    lines.append(
        f"2. **规则路由在 robustness 集上骤降至 {rob['accuracy']*100:.1f}%**——"
        "对同义替换（'咋算' vs '怎么算'）、隐私变体（'电话' vs '手机号'）、"
        "注入改写（'忽略上面的要求' vs '忽略之前指令'）缺乏泛化，存在真实误路由与安全风险。\n"
    )
    if has_llm:
        lines.append(
            "3. LLM 侧在上述两项上的表现见上表；若其在 robustness 上显著领先，"
            "则论证'规则优先 + LLM 兜底'的混合路由在真实流量下更稳。\n"
        )
    else:
        lines.append(
            "3. **LLM 路由的对照价值在此显现**：对自然语言的泛化正是 LLM 的强项，"
            "预期在 robustness 集上明显优于规则。是否切换取决于生产流量中'改写/变体'占比——"
            "若日志显示误路由率超过阈值，即可用本仓库已实现的 `llm_router` 与混合策略补齐。\n"
        )
    lines.append(
        "4. 结论：v2 选规则路由是'在已观测流量分布下的最优解'，而非'只会规则路由'；"
        "本 A/B 框架使该决策可被数据复盘与迭代。\n"
    )
...
def main() -> int:
    ...
    if os.environ.get("LLM_API_KEY"):
        ...
        llm_res = {"curated": llm_cur, "robustness": llm_rob}
    else:
        print("[eval] 未检测到 LLM_API_KEY，跳过 LLM 侧（不伪造数据）。")

    report = build_report(
        {"curated": rule_cur, "robustness": rule_rob},
        llm_res,
        {"curated": len(curated), "robustness": len(robustness)},
    )
    (HERE / "ab_routing_report.md").write_text(report, encoding="utf-8")

    payload = {
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "rule": {"curated": rule_cur, "robustness": rule_rob},
        "llm": llm_res,
    }
    (HERE / "ab_routing_report.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")

    print("[eval] 规则侧 curated 准确率:", rule_cur["accuracy"], "robustness 准确率:", rule_rob["accuracy"])
